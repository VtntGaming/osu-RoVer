...
return function(FileType,Beatmap,SongSpeed,DelayedTime,CustomSongIDEnabled,isReturnDifficulty,metadataonly,GetPS,IsHR,isFL,IsEZ,IsHD)
	...
	if metadataonly ~= true then -- this would reduce lag
		for objectID,HitObj in pairs(MapData["HitObjects"]) do
			...
			local function GetSpeedDiff(Speed)  -- Aim diff
				...
				return math.min(math.pow(Speed,0.4)/14,2) --ReturnDiff
			end
			...
			local function GetStreamDiff(TimeBetween)  -- Speed diff .-.
				...
				ReturnDiff = math.min(math.pow(1/TimeBetween,0.5)*6.5,1)
			...
			local function GetVelocityChangeBonus(CurrentVelocity,MoveTime)
				...
				VelocityChangeBonus *= math.pow(math.min(MoveTime,PrevMoveTime)/math.max(MoveTime,PrevMoveTime),2)
			...
			if isReturnDifficulty == true then
				...
				if objectID ~= 1 and not isSpinner then
					...
					--RawDifficulty[#RawDifficulty+1] = {Difficulty,Time}
					RawDifficulty[#RawDifficulty+1] = {math.pow(CurrentStrain,0.5)*2.35,Time}
	...
	if isReturnDifficulty == true then
		...
		BeatmapDifficulty = math.pow(HighestStrain,0.5)*2.35 -- NEW DIFFICULTY CALCULATOR

		BeatmapDifficulty = math.floor(BeatmapDifficulty*100+0.5)/100
		--print(math.pow(HighestAimStrain,0.5)*2.35,math.pow(HighestSpeedStrain,0.5)*2.35)
