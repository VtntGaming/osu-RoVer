	...
	if metadataonly ~= true then -- this would reduce lag
		for objectID,HitObj in pairs(MapData["HitObjects"]) do
			...
			local function GetAngleBonus(Angle,CurrentMoveTime,PrevMoveTime,lengthprev,lengthcurrent)
				...
				--local prevVector = {Length = 100} local currVector = {Length = 200} local osuCurrObj = {JumpDistance = 100}
				
				--print(math.max(0,math.sqrt((prevVector.Length - currVector.Length) * currVector.Length) - currVector.Length) * math.min(1, osuCurrObj.JumpDistance / 100))
				...
				AngleBonus = 0 -- this is not ready, lol
						...
						for i = #ConvertedData-1,1,-1 do
							...
							if (TimeBetween < 200*(Count) and Count < 20) or Count == 1 then
								...
								if i ~= #ConvertedData then
									...
									local CurrentStreamDifficulty = GetStreamDiff(TimeBetween) - GetStreamMultiplier(#ConvertedData,i)
			...
			local HitObjData = {Position = HitPos,Time = Time,Type = Type,ExtraData = ExtraData,SpinTime = SpinTime,PSValue = RewardPS}
